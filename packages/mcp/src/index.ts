  ...
  if (!registerLegacy) {
    ...
    registerTool(
      ...
      "Confirm a vault exists on-chain and save its address to config. Use after signing the vault creation Blink to populate the vault address.",
      {
        owner: z
          ...
          .optional()
          .describe(
            "Owner public key (base58). Defaults to configured wallet.",
          ),
        vaultId: z
          .number()
          .optional()
    ...
    registerTool(
      ...
      "Check the status and policy configuration of an Phalnx vault",
      {
        vault: z
          .string()
          .describe(
            "Vault PDA address (base58). Provide this OR owner+vaultId.",
          ),
        owner: z
          .string()
          .optional()
    ...
    registerTool(
      ...
      "Update the policy configuration for a vault (owner-only)",
      {
        ...
        maxLeverageBps: z
          .number()
          .optional()
          .describe("New max leverage in BPS"),
        canOpenPositions: z
          .boolean()
          .optional()
    ...
    registerTool(
      ...
      "Update the permission bitmask for a registered agent. Owner-only.",
      {
        ...
        permissions: z
          .string()
          .describe(
            "New permission bitmask as decimal string (full = 2097151)",
          ),
      },
    ...
    registerTool(
      ...
      "Place a TP/SL trigger order on a Flash Trade position (non-spending)",
      {
        ...
        receiveSymbol: z
          .string()
          .describe("Token symbol to receive on trigger"),
        side: z.enum(["long", "short"]).describe("Position side"),
    ...
    registerTool(
      ...
      requireClient((input) =>
        x402Fetch(client!, config!, input, custodyWallet),
      ),
    );
