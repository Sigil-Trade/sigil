...
export interface BuildUnpauseAgentTxParams {
  owner: PublicKeyType;
  agentToUnpause: PublicKeyType;
  vaultId: number;
}

export async function buildUnpauseAgentTransaction(
  params: BuildUnpauseAgentTxParams,
  ...
  const unpauseIx = await buildUnpauseAgent(
    program,
    params.owner,
    vaultPDA,
    params.agentToUnpause,
  ).instruction();
  ...
  const ixs = priorityFeeIx
    ? [computeIx, priorityFeeIx, unpauseIx]
    : [computeIx, unpauseIx];
