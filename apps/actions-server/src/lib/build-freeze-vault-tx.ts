...
export interface BuildFreezeVaultTxParams {
  owner: PublicKeyType;
  vaultId: number;
}

export async function buildFreezeVaultTransaction(
  params: BuildFreezeVaultTxParams,
  ...
  const { IDL, buildFreezeVault, getVaultPDA, CU_OWNER_ACTION, getEstimator } =
  ...
  const freezeIx = await buildFreezeVault(
    program,
    params.owner,
    vaultPDA,
  ).instruction();
  ...
  const ixs = priorityFeeIx
    ? [computeIx, priorityFeeIx, freezeIx]
    : [computeIx, freezeIx];
