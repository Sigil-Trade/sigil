...
describe("agent-errors", () => {
  ...
  describe("ON_CHAIN_ERROR_MAP completeness", () => {
    it("maps all 112 error codes (6000-6111) post H-1", () => {
      ...
      expect(codes).to.have.lengthOf(112);
      expect(codes[0]).to.equal(6000);
      expect(codes[codes.length - 1]).to.equal(6111);
    });

    it("every code from 6000-6111 is present with no gaps post H-1", () => {
      for (let code = 6000; code <= 6111; code++) {
