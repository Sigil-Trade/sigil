...
## §8 — REVISED DESIGN v2 (SUPERSEDES §1+§2; incorporates Kaleb override + §7 review + Flash/prior-art feasibility, 2026-06-05)

**Override resolved (Kaleb): owning Jupiter/Flash positions is NON-NEGOTIABLE.** → Path = **HYBRID**: a minimal-authority **SCOPED EXECUTOR** (vault PDA `invoke_signed`s the open) for non-delegate venues (Jupiter/Flash) + **native-delegate** for Drift/Mango/Parcl. The non-custodial SPL-Approve spine stays for swaps/transfers.

**Conceptual spine — "honeypot" is TWO leaks (prior-art verdict; no one eliminates both):**
- **EXIT honeypot** (controlling program sends funds where IT chooses) — **CLOSABLE on-chain** by pinning exit destination == owner.
- **ACTION honeypot** (bleed value via legit-looking bad/self-dealing trades) — **NOT closable on-chain** for an open venue (a valid trade and a self-deal are indistinguishable to the signer); bounded by caps/floor/monitoring, never zero.
- **Universal mechanism (Drift #1 + everyone): separate WHO-MAY-ACT from WHO-MAY-EXIT; pin EXIT to owner in program code.** The frontier shrinks the residual to "owner-pinned exit + bounded action." Sigil matches/beats best-in-class (Drift) via per-user + on-chain exit-pin + the mandatory floor. **The "boondoggle/massive attack vector" = a POOLED GENERAL executor (GLAM-class); Sigil is per-user + scoped → that's avoided by construction.**

**Design components:**
1. **Per-user isolation** (already built) — no shared pool. The dangerous honeypot axis is designed out.
2. **Scoped executor (NEW):** a Sigil instruction that constructs + `invoke_signed`s the EXACT validated perp-open — **program-id-pinned + discriminator-pinned + specific accounts** — for an allowlisted venue, vault PDA as `owner`. NOT a general "sign any instruction" executor. The agent supplies ONLY trade params (size / collateral / price-with-slippage), never the program or value-bearing accounts. (This is the CRITICAL-2 fix: exact-ix binding, not the 5-scalar digest.)
3. **EXIT-pin (closes the exit honeypot), per venue:**
   - **FLASH = CONFIRMED-CLEAN:** Flash's own `has_one = owner` pins `funding_account` + `receiving_account` + `position` PDA all to owner(=vault). Exit closed by the venue's constraints; the agent cannot redirect funding or payout. Sigil-wrapped `open_position` peaks at CPI stack height **3/5** (CONFIRMED, 2 levels headroom). BEST CASE. (2 v15 closed-source confirms outstanding: `owner` still a bare `Signer`; no extra nested CPI in v15 open — Kaleb can supply.)
   - **JUPITER = FEASIBLE, gated on one empirical test (T3).** DEPTH: shallow phase-1 `createIncreasePositionMarketRequest` peaks at stack height **H4** (no oracle accounts; ATA-create is the deepest leg) — FITS, but **ONLY if Sigil's instruction is TOP-LEVEL** (sub-CPI'ing Sigil shifts +1 → overflow → brick; design constraint §8.8). PDA-OWNER: **CONFIRMED** — Jupiter *deliberately refactored* `SystemAccount`→`UncheckedAccount` program-wide (OtterSec OS-JPT-ADV-04 + Offside PR-65) *specifically to accept PDA owners* → `owner = vault PDA` ⇒ `Position.owner = vault PDA`. EXIT: closes **TRANSITIVELY through ownership** (LIKELY-HIGH) — `PositionRequest` stores NO free-form destination (only `owner`+`mint`); the keeper derives payout `ATA(owner,mint) = ATA(vault,mint)`; Jupiter already enforces ATA-derivation on-chain for the escrow ata (Offside §4.4). **Single residual:** whether `closePositionRequest2/3` enforces `ownerAta == ATA(position.owner, mint)` on-chain (not provable from IDL+audits) → **T3 MUST pass before custody** (drive a keeper-fill close with attacker `ownerAta` ≠ owner; assert REJECT). NOTE: position materializes **async via the keeper** (not at end of Sigil's tx); there is **NO synchronous owner-only close** — safety rests on OWNERSHIP propagating to the keeper's derivation, NOT Sigil presence. **Stays on the fail-closed `KNOWN_ASYNC_FULFILLMENT_PROGRAMS` denylist until T3 passes.**
4. **OWNER-PIN (re-instated M3-02 — closes keeper-settles-to-agent):** finalize-time per-protocol `Position.owner == vault` assertion (`verify_protocol_account_owner_pin`). PER-VENUE (offset per IDL) — explicitly NOT agnostic for the perp lane. (CRITICAL-2 fix.)
5. **MANDATORY conservation floor (bounds the action honeypot + CRITICAL-1 fix):** bound to `target_protocol` via a program-curated per-protocol required-assertion table, enforced at `validate`. Swap lanes: slippage-relative floor (`value-out ≥ value-in × (1 − max_slippage_bps)`, NOT `>0`). Perp lanes: the collateral-out cap (the action honeypot is bounded, not closed). New err **6111 `ErrConservationFloorMissing`**. Test: a 1-lamport-return swap REVERTS.
6. **No standing pool:** collateral flows owner-ATA → (atomic) → vault-owned position AT THE VENUE; value lives at the venue per-user, never in a Sigil treasury. No standing Sigil balance to drain.
7. **CPI-depth:** wrap the SHALLOW leg (Flash `open_position` 3/5 CONFIRMED; Jupiter phase-1 verifying); depth violation fails-CLOSED (whole tx reverts).

8. **EXECUTOR DESIGN CONSTRAINTS (from Jupiter feasibility):** (i) **Sigil-top-level-only** for the executor path — Sigil's wrapping instruction MUST be a top-level tx instruction, never reachable via sub-CPI (e.g. Squads→Sigil), which shifts the depth ladder +1 and bricks the wrapped open. (ii) **Guard re-engineered for the wrap model** — Sigil's spine guard reads the instructions sysvar to introspect a *sibling* DeFi ix; that does NOT carry into the `invoke_signed`-wrap. The executor instead CONSTRUCTS the exact ix, so it validates *what it builds* (program + discriminator + accounts) + runs the owner-pin/conservation checks in-program before/after the CPI — a distinct code path from the spine.

**Honest residual (what NO ONE solves):** the ACTION honeypot — a trade-capable agent can bleed value via bad/self-dealing trades, bounded by the spend cap + the mandatory floor + off-chain monitoring, never zero. Drift, drift-vaults (admitted verbatim: "risk of draining the vault by placing losing trades… no on-chain method for recourse"), and every protocol accept this. Sigil's posture = best-in-class.

**New threat surface (disclosed):** Sigil gains a scoped `invoke_signed`-perp-open capability — contained by exact-ix construction + Flash's `has_one` / Sigil's owner-pin + exit-pin (Jupiter) + per-user isolation + the mandatory floor. NOT a general executor.

**Per-venue verdict:** **Flash = FEASIBLE + clean** (exit closed by `has_one`; depth 3/5). **Jupiter = FEASIBLE, gated on T3** (depth fits H1-only; PDA-owner CONFIRMED; exit closes transitively; one keeper-fill `ownerAta` test before custody). **Drift/Mango/Parcl = native-delegate** (Mango action-residual flagged). Error codes append-only from 6111.

**Pre-custody empirical gates:** **T1** mainnet-fork depth (Sigil top-level; max stack ≤ depth-4 + CU); **T2** devnet PDA-owner (`Position.owner == vault`); **T3 (decisive)** Jupiter keeper-fill `ownerAta` pin (attacker `ownerAta` ⇒ REJECT); + 2 Flash v15 closed-source confirms (`owner` still a bare `Signer`; no extra nested CPI in v15 `open_position` — Kaleb can supply).

**STATUS:** ALL feasibility RESOLVED — Flash CONFIRMED-clean; Jupiter FEASIBLE gated on T3; Drift/Mango/Parcl native-delegate. §7 review (2 CRITICALs) + prior-art (two-honeypot verdict) folded in. NEXT: implement under the mandatory pipeline — H1 mandatory floor → owner-pin (re-instated M3-02) → scoped executor (Flash first; Jupiter after T3) → H2 (reject PermanentDelegate) → H3 (anchor-idl-guard).
