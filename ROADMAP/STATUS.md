...
> **▶ FOLDED INTO `UNIFIED_PLAN_2026-06-06.md` (the current forward plan) — read that first.** This ledger is the **M1 on-chain base: functionally DONE and preserved as the foundation** — it IS the demand-validated trustless control layer (allowlist + spend caps + frozen-gate + agnostic primitives + M3-01 floor). Forward priority has shifted to the **outward product layer** this plan deliberately deferred (audit-export wedge, activity, positions, control-UX). M2/M3's remaining on-chain ambitions are largely **superseded by the agnostic/vault-conservation reset** — see the unified plan §2 for the item-by-item reconciliation. Still owed to *close* this plan: the **M1-EXIT gate** (§4) + merge/CI the branch.
...
**Last reconciled:** 2026-06-04 (HEAD `3e2e84e0`). NOTE: two roadmap framings now coexist — this ledger's original `M1-XX` items, and the 2026-06-01 audit's `F-Qx` items in `ENFORCEMENT_MODEL.md` (the authoritative enforcement spec). The mapping is noted inline (M1-01=F-Q1a, M1-05≈F-Q8, M1-06≈F-Q4); a full structural merge of the two framings is pending doc-debt.
**Shipped since last recon (all on this branch):** F-Q1a `3ed911bd`, F-Q8 output-ATA pin `bedd7dfe`, F-Q4 Token-2022 swap-gate + layout fix `b710f6a9` + SDK output-mint satisfier `b6503fa4`, F-Q6 OPERATOR timelock `4ef4ae26`/`687ebec3`/`ec40ecbb`/`8d9732fc`/`0fda650d`, F-Q9 checked-revert `76e0ccc7`, F-Q2 `defi_ix_count==1` `39c0c608`, program-ID sweep `d7d476e5`, M3-01 stable-floor canonical-ATA pin + SDK satisfier `67e2b856`/`6534dba5`/`66a6ee83`.
**Current test baseline (HEAD `3e2e84e0`):** full LiteSVM **503 passing / 5 pending / 0 failing**; kit **1868 passing / 0 failing**; cargo **166 passing / 0 failing**. (Supersedes the 148/2/0 and 544/12/5 framings in the §Test-suite reality block, which describe earlier HEADs.)
