...
> **▶ STATUS SNAPSHOT (updated 2026-07-01).** The **M1 / M2 / M3 on-chain hardening milestones are COMPLETE and merged to `main`.** The foundation — the TypeScript SDK plus the on-chain program — was independently reviewed and assessed **SOUND** (no blocking findings). Sigil's demand-validated trustless control layer (destination allowlist + spend caps + frozen-gate + protocol-agnostic primitives + the M3-01 stable-balance-floor backstop) is the shipped base; forward priority has shifted to the **outward product layer** (audit-export wedge, activity feed, positions, control-UX).
>
> **Currently-shipped model — source of truth is the code, NOT this ledger:** the owner grants spend authority via **SPL-`Approve` delegation** (the token program's `delegated_amount` is the hard cap); per-agent authority is a **2-bit capability** (DISABLED / OBSERVER / OPERATOR); enforcement is the atomic `validate_and_authorize → DeFi ix → finalize_session` sandwich, outcome-based on measured stablecoin balance-delta. The program has **32 instructions**, **118 error codes (6000–6117, append-only)**, and **12 PDA account types**. Program ID `7FtAXUcrann7P5HoLG7vnWcVpozwj9nqcNm6bPwA1wuK` (upgrade-in-place, no new ID). Note: the "vault-signs-everything / no-delegation" design in `SIGIL_V2_AUTHORITATIVE_ARCHITECTURE_2026-06-06.md` is a **deferred future** direction, NOT what is shipped.
>
> **Doc-cleanup note (why some cross-references below no longer resolve):** a documentation cleanup **archived out of the repo** the `M1-XX`-step `ROADMAP/` files, `M3_ASYNC_HARDENING.md`, `docs/revamp/`, `docs/review/`, `CONTRIBUTING.md`, `docs/DEPLOYMENT.md`, and most of the rest of `docs/` — of the old `docs/` tree only `docs/ERROR-CODES.md` survives as a tracked file (git history retains everything removed). Still present as sibling living docs in this `ROADMAP/` folder: `UNIFIED_PLAN_2026-06-06.md`, `ENFORCEMENT_MODEL.md`, `ENFORCEMENT_MODEL_DECISIONS.md`, `SIGIL_V2_AUTHORITATIVE_ARCHITECTURE_2026-06-06.md` (a **deferred future** design — not shipped), `MULTI_ASSET_GUARDS_DESIGN_TRACK_2026-06-17.md`, and `COSIGN_ASYNC_APPROVAL_DESIGN_2026-06-17.md`. Any filename reference in the tables below that no longer resolves to a tracked path is historical.
>
...
- Docs archive: DONE (2026-07) — historical `docs/revamp/`, `docs/review/`, M1-step ROADMAP files, `CONTRIBUTING.md`, `docs/DEPLOYMENT.md`, and most of `docs/` removed from the repo (git history retains them)
...
| M1-06 mediums (= F-Q4) | **CODE DONE; residuals open** | `b710f6a9` / `b6503fa4` | **F-Q4 CODE SHIPPED.** F-Q4c (`b710f6a9`): NonTransferable(9) REMOVED from the extension allowlist (`token2022_extension.rs` — live accept-set is now MemoTransfer(8) + MetadataPointer(18) only; tier-1 brick gate) + layout fix. F-Q4b (`b710f6a9` on-chain + `b6503fa4` SDK): extension-allowlist gating extended to the finalize typed chokepoint (writable vault-owned Token-2022 output ATAs) with err `6106 ErrToken2022OutputMintUnresolvable` propagation; `seal()` resolves vault-owned Token-2022 output mints (honest path). **STILL OPEN (residual mediums, not F-Q4):** MED-1 — non-stablecoin *input* upper-bound (see M1-05/F-Q8 reframe; tracked as the valuation-wall residual); MED-2 — per-protocol cap rolling-vs-boundary (fix A rolling vs B accept; see Open decisions). NOTE: **RESOLVED** (doc-debt cleanup) — the 6079 `ErrToken2022ExtensionForbidden` doc + `#[msg]` (plus IDL/types mirror, SDK error maps, `docs/ERROR-CODES.md`) corrected from "3 extensions incl NonTransferable" to "MemoTransfer + MetadataPointer only". Live allowlist source `token2022_extension.rs` was already correct (unit test `non_transferable_rejects`). Pre-existing prettier debt on `agent-errors.generated.ts` (generator output, not in `.prettierignore`) surfaced — see Open decisions. Remaining stale "allow NonTransferable" copies live ONLY in historical `docs/revamp/` (5 sites; left as superseded archive). |
...
- **Full first-principles enforcement audit + 5-perspective council + my verification** (2026-06-01)
  → `ENFORCEMENT_MODEL.md` + `ENFORCEMENT_MODEL_DECISIONS.md` (authoritative spec; now tracked in-repo, verified via `git ls-files ROADMAP/`).
  Re-grounds M1-01..M2 as `F-Qx`. Kaleb ratified Q1-Q9 with overrides; verification right-sized
  inflated severities (F-Q2 refuted-as-drain, deferred then SHIPPED `39c0c608` as defense-in-depth; F-Q6 HIGH-not-CRIT, SHIPPED; F-Q9 LOW, fixed `76e0ccc7`).
- **Shipped findings (code) — status roll-up:**
  - **F-Q1a** destination completeness + seal satisfier — **DONE** `3ed911bd` (M1-01 row).
  - **F-Q2** `defi_ix_count == 1` (free-delegation window) — **DONE** `39c0c608` (status-recon `2687ddae`).
  - **F-Q4** Token-2022 swap-path gate + NonTransferable removed + SDK satisfier — **DONE** `b710f6a9` + `b6503fa4` (M1-06 row).
  - **F-Q6** OPERATOR-grant timelock (A1 digest-bind `4ef4ae26`, A2 owner_type `687ebec3`, B1 owner-configurable delay `ec40ecbb`, tier model + uniform enforcement `8d9732fc`, adversarial-pass hardening `0fda650d`) — **DONE**. Closes the instant-OPERATOR-on-non-cosign-vault gap this audit found.
  - **F-Q8** output-ATA pin — **DONE** `bedd7dfe` (M1-05 row; min-out/input-bound/digest-fold REJECTED as oracle/scope creep — documented residual).
  - **F-Q9** finalize spend-accounting checked-revert (err 6110 `SpendAccountingUnderflow`) — **DONE** `76e0ccc7` (docs-recon `e06984dd`); Certora conservation proof remains M2.
...
| Item | Status | PR | Notes |
|---|---|---|---|
| M3-01 floor catastrophic backstop | **DONE** | `67e2b856` / `6534dba5` / `66a6ee83` | Combined USDC+USDT `stable_balance_floor` now **fed on seal + canonical-ATA-pinned**. `67e2b856`: finalize pins each stablecoin balance to its canonical (derivable) vault ATA (a non-canonical vault-owned account funded above the floor is ignored → still reverts, proving the pin); err **6085 `ErrStableFloorViolation`** (relabeled from a mis-cited 6094 in `66a6ee83`). `6534dba5`: `seal()` satisfier feeds the vault's *other* stablecoin ATA into finalize's `remaining_accounts` (Source 3), completing the combined-sum floor on the honest path (warn-and-proceed on RPC miss — a floor-miss is fail-safe/over-strict, never loss of funds). **Change (a) "forced/default-on onboarding" was REJECTED by Kaleb — the floor stays OPT-IN, never forced** (default `stable_balance_floor = 0` = skip; on-chain enforcement is the sole enforcer when set). Evidence: full LiteSVM **503/5/0**, kit **1868/0**, cargo **166/0**. Depends on F-Q1a (done). |
